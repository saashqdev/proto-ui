...
| --------- | ---- | ------- | ------- | ----------- |
| gap | string | | `'normal'` | The gap between rasters, one value (column interval) or two values   (row interval column interval). Optional values: `'none'`, `'small'`, `'normal'`, `'large'` |

### Slots

#### (default)

Insert `<u-grid-layout-row>` or `<u-grid-layout-column>` subcomponent.
...
| Prop/Attr | Type | Options | Default | Description |
| --------- | ---- | ------- | ------- | ----------- |
| repeat | number | | `12` | Number of divided grids |
| type | string | | | Layout mode. Optional value: `'flex'` or leave blank |
| justify | string | | `'start'` | The horizontal distribution method of flex layout mode, optional values: `'start'`, `'center'`, `'end'`, `'space-between'`, `'space-around'` |
| alignment | string | | `'stretch'` | Vertical alignment of flex layout mode, optional values: `'start'`, `'center'`, `'end'`, `'baseline'`, `' stretch'' |
